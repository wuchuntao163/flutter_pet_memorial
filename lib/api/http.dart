...
class _Http {
  ...
  Future<ApiResponse<T>> _request<T>(
    ...
    try {
      ...
      return _parseBody<T>(_parseJson(res.data), parser: parser);
    } on ApiException {
      rethrow;
  ...
  ApiResponse<T> _parseBody<T>(
    ...
    if (payload is Map && payload['k'] != null && payload['r'] != null) {
      ...
      return ApiResponse(code: code, msg: msg, data: parsed, fromEncrypted: true);
    }

    return ApiResponse.fromJson(body, parser: parser);
  }
    ...
    if (raw != null) {
      try {
        ...
        handler.reject(DioException(
          ...
          type: err.type,
          error: ApiException.business(
            _asInt(map['code']),
            map['msg']?.toString() ?? '请求失败!!!',
            map['data'],
          ),
        ));
        return;
